...
import { useStore, EnergyEntry } from '../store';
import { Colors, Spacing, Radius, Typography, Shadow } from '../theme';
import { Card, Button, Label, SectionHeader, EnergyBar, Divider } from '../components/ui';
...
type ActivityType = EnergyEntry['activityType'];
...
function HealthImportBanner({ onImport }: { onImport: () => void }) {
  return (
    <Card style={styles.healthBanner}>
      ...
      <View style={styles.healthSourceRow}>
        {['Apple Health', 'Google Fit', 'Fitbit', 'Oura', 'Garmin'].map(src => (
          <TouchableOpacity key={src} style={styles.healthSource} onPress={onImport}>
...
export default function EnergyScreen({ navigation }: Props) {
  ...
  const [tab, setTab] = useState<'today' | 'log' | 'trends'>('today');
  const [customActivity, setCustomActivity] = useState('');
  const [customCost, setCustomCost] = useState('');
  const [customType, setCustomType] = useState<ActivityType>('other');
  ...
  function handleImportHealth() {
    Alert.alert(
      'Connect health data',
      'In the full app, this connects to Apple Health (iOS), Google Health Connect (Android), or Garmin/Oura via their APIs to read:\n\n• Sleep duration & quality\n• Resting heart rate\n• HRV (stress indicator)\n• Steps / activity\n\nThis data would auto-adjust your daily energy baseline.',
      [
  ...
  function handleLogPreset(idx: number) {
  ...
  function handleLogCustom() {
    ...
    if (!customActivity.trim() || isNaN(cost)) return;
    addEnergyEntry({
      timestamp: Date.now(),
  ...
  const capacityLabel =
    currentEnergy >= 70 ? 'Good capacity for demanding tasks'
  ...
  return (
    <View style={styles.container}>
      <View style={styles.tabs}>
        {(['today', 'log', 'trends'] as const).map(t => (
        ...
        {tab === 'today' && (
          <>
              ...
              <View style={styles.manualRow}>
                ...
                <View style={styles.manualBtns}>
                  {[-10, -5, +5, +10].map(delta => (
                    <TouchableOpacity
                      key={delta}
                      style={[styles.manualBtn, { backgroundColor: delta < 0 ? Colors.dangerLight : Colors.successLight }]}
                      onPress={() => setCurrentEnergy(currentEnergy + delta)}
                    >
            ...
            {todayEntries.length === 0 ? (
              <Card><Text style={styles.emptyText}>No activities logged yet. Use the Log tab.</Text></Card>
            ) : (
        ...
        {tab === 'log' && (
          <>
            ...
            <View style={styles.presetGrid}>
              {ACTIVITY_PRESETS.map((p, idx) => (
                <TouchableOpacity
                  key={idx}
                  style={[styles.presetBtn, { borderLeftColor: TYPE_COLORS[p.type] }]}
                  onPress={() => handleLogPreset(idx)}
                  activeOpacity={0.7}
                >
            ...
            <Card>
              ...
              <TextInput
                ...
                placeholder="Energy cost (e.g. -15 or +20)"
                placeholderTextColor={Colors.textMuted}
                value={customCost}
              ...
              <View style={styles.typeRow}>
                {(['rest', 'work', 'social', 'travel', 'personal'] as ActivityType[]).map(t => (
                  <TouchableOpacity
                    key={t}
                    style={[styles.typeBtn, customType === t && { backgroundColor: TYPE_COLORS[t] + '20', borderColor: TYPE_COLORS[t] }]}
              ...
              <Button label="Log this activity" onPress={handleLogCustom} fullWidth style={{ marginTop: Spacing.md }}
                disabled={!customActivity.trim() || !customCost.trim()} />
            </Card>
          </>
        )}
...
function EntryRow({ entry }: { entry: EnergyEntry }) {
...
function ActivityBreakdown({ entries }: { entries: EnergyEntry[] }) {
        ...
        <View key={type} style={styles.breakdownRow}>
          ...
          <View style={styles.breakdownBar}>
            <View style={[styles.breakdownFill, {
              width: `${(val / total) * 100}%` as any,
              backgroundColor: TYPE_COLORS[type as ActivityType] || Colors.textMuted,
            }]} />
          </View>
...
function TopDrains({ entries }: { entries: EnergyEntry[] }) {
  const drains = entries.filter(e => e.cost < 0)
    .sort((a, b) => a.cost - b.cost)
    .slice(0, 5);
  if (drains.length === 0) return <Text style={styles.emptyText}>No draining activities logged yet.</Text>;
  return (
    <View style={{ gap: 6 }}>
      {drains.map(e => (
        <View key={e.id} style={styles.drainRow}>
