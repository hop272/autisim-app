...
export function Card({ children, style, onPress }) {
    if (onPress) {
        return (<TouchableOpacity style={[styles.card, style]} onPress={onPress} activeOpacity={0.8}>
        {children}
      </TouchableOpacity>);
    }
    return <View style={[styles.card, style]}>{children}</View>;
}
export function Button({ label, onPress, variant = 'primary', size = 'md', loading, disabled, style, fullWidth, }) {
    const btnStyle = [
        styles.btn,
        styles[`btn_${variant}`],
        styles[`btn_${size}`],
    ...
    return (<TouchableOpacity style={btnStyle} onPress={onPress} disabled={disabled || loading} activeOpacity={0.75}>
      {loading ? (<ActivityIndicator color={variant === 'secondary' || variant === 'ghost' ? Colors.primary : Colors.textInverse} size="small"/>) : (<Text style={[styles.btnText, styles[`btnText_${variant}`], styles[`btnText_${size}`]]}>
          {label}
        </Text>)}
    </TouchableOpacity>);
}
export function Label({ text, variant = 'default', style }) {
    return (<View style={[styles.label, styles[`label_${variant}`], style]}>
...
export function EnergyBar({ value, max = 100, showLabel, height = 10, style }) {
    ...
    return (<View style={style}>
      ...
      {showLabel && (<Text style={[styles.energyBarLabel, { color }]}>
          {Math.round(pct)}%
        </Text>)}
    </View>);
}
export function SliderRow({ label, value, max = 10, onValueChange, color = Colors.primary }) {
    const steps = Array.from({ length: max }, (_, i) => i + 1);
    return (<View style={styles.sliderRow}>
      <Text style={styles.sliderLabel}>{label}</Text>
      <View style={styles.sliderDots}>
        {steps.map(step => (<TouchableOpacity key={step} onPress={() => onValueChange(step)} style={[
                styles.sliderDot,
...
export function EmptyState({ emoji, title, body, action }) {
    return (<View style={styles.empty}>
      ...
      {action && (<Button label={action.label} onPress={action.onPress} variant="secondary" style={{ marginTop: Spacing.md }}/>)}
    </View>);
}
// ── Styles ────────────────────────────────────────────────────────────────────
