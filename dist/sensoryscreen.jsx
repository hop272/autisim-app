...
import { Card, Button, Label, SectionHeader, SliderRow } from '../components/ui';
const HOUR = 3600000;
function timeAgo(ts) {
...
export default function SensoryScreen({ navigation }) {
    ...
    return (<View style={styles.container}>
      ...
      <View style={styles.tabs}>
        {['log', 'history', 'patterns'].map(t => (<TouchableOpacity key={t} style={[styles.tab, tab === t && styles.tabActive]} onPress={() => setTab(t)}>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        {tab === 'log' && (<LogForm noise={noise} setNoise={setNoise} light={light} setLight={setLight} crowding={crowding} setCrowding={setCrowding} smell={smell} setSmell={setSmell} temperature={temperature} setTemperature={setTemperature} clothing={clothing} setClothing={setClothing} mood={mood} setMood={setMood} energy={energy} setEnergy={setEnergy} location={location} setLocation={setLocation} note={note} setNote={setNote} eventType={eventType} setEventType={setEventType} onSave={handleSave} saved={saved}/>)}

        {tab === 'history' && (<View>
            <SectionHeader title="Check-in history" subtitle={`${sensoryLogs.length} entries`}/>
            {sensoryLogs.length === 0 ? (<View style={styles.empty}>
                <Text style={styles.emptyText}>No logs yet. Use the Log tab to check in.</Text>
              </View>) : (sensoryLogs.map(log => (<LogCard key={log.id} log={log}/>)))}
          </View>)}

        {tab === 'patterns' && (<View>
            <SectionHeader title="Patterns detected" subtitle={`Based on ${sensoryLogs.length} check-ins`}/>
            {patterns.length === 0 ? (<Card>
              ...
              </Card>) : (patterns.map((p, i) => (<Card key={i} style={styles.patternCard}>
    ...
    return (<View>
      ...
      <Card style={styles.section}>
        ...
        <View style={styles.eventTypeRow}>
          {eventTypes.map(et => (<TouchableOpacity key={et.key} style={[
                styles.eventTypeBtn,
      ...
      <Card style={styles.section}>
        ...
        <TextInput style={styles.textInput} placeholder="Where are you? (e.g. supermarket, home, work)" placeholderTextColor={Colors.textMuted} value={props.location} onChangeText={props.setLocation}/>
        <TextInput style={[styles.textInput, { marginTop: Spacing.sm, minHeight: 60 }]} placeholder="Any notes…" placeholderTextColor={Colors.textMuted} value={props.note} onChangeText={props.setNote} multiline/>
      </Card>

      <Button label={props.saved ? '✓ Saved' : 'Save check-in'} onPress={props.onSave} fullWidth variant={props.saved ? 'secondary' : 'primary'} style={{ marginTop: Spacing.sm }}/>
    </View>);
}
// ── Log card ──────────────────────────────────────────────────────────────────
function LogCard({ log }) {
    const { text: noiseLabel, variant: noiseVariant } = intensityLabel(log.noise);
    return (<Card style={styles.logCard}>
      <View style={styles.logCardHeader}>
        ...
        {log.eventType && (<Label text={log.eventType} variant={log.eventType === 'regulated' ? 'success'
      ...
      <View style={styles.logCardGrid}>
        {[
            ...
            return (<View key={item.label} style={styles.logGridCell}>
...
function SensoryHeatmap({ logs }) {
    if (logs.length === 0)
        return <Text style={styles.patternEmpty}>No data yet.</Text>;
    const fields = [
    ...
    return (<View style={{ gap: 10 }}>
      {fields.map(f => {
            const vals = logs.map(l => l[f.key]).filter(v => typeof v === 'number');
            ...
            return (<View key={f.key} style={styles.heatRow}>
            <Text style={styles.heatLabel}>{f.label}</Text>
            <View style={styles.heatBar}>
              <View style={[styles.heatFill, { width: `${avg * 10}%`, backgroundColor: barColor }]}/>
            </View>
