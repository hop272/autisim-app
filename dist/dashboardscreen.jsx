...
function energyDescription(e) {
    if (e >= 70)
        return 'Good capacity for demanding tasks';
    if (e >= 40)
        return 'Enough for 1–2 medium tasks';
    if (e >= 20)
        return 'Light tasks only — rest when you can';
...
export default function DashboardScreen({ navigation }) {
    ...
    const energyColor = currentEnergy >= 70 ? Colors.energyHigh
    ...
    return (<ScrollView style={styles.scroll} contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity style={[styles.recoveryBtn, isRecoveryMode && styles.recoveryBtnActive]} onPress={() => navigation.navigate('Recovery')}>
      ...
      <Card style={styles.energyCard}>
        ...
        <TouchableOpacity style={styles.energyLink} onPress={() => navigation.navigate('Energy')}>
      ...
      {activeTask ? (<View>
          ...
          <Card onPress={() => navigation.navigate('Tasks')}>
            ...
            {activeTask.steps[activeTask.currentStepIndex] && (<View style={styles.stepNow}>
          ...
          <TouchableOpacity style={styles.emptyTask} onPress={() => navigation.navigate('Tasks')}>
      ...
      <View style={{ marginTop: Spacing.md }}>
        <SectionHeader title="Sensory snapshot" subtitle={latestSensory ? getTimeAgo(latestSensory.timestamp) : undefined} action={{ label: 'Log now', onPress: () => navigation.navigate('Sensory') }}/>
        {latestSensory ? (<Card>
            ...
            ].map(item => (<View key={item.label} style={styles.sensoryCell}>
            ...
            {latestSensory.location && (<Text style={styles.sensoryLocation}>📍 {latestSensory.location}</Text>)}
          </Card>) : (<TouchableOpacity style={styles.emptyTask} onPress={() => navigation.navigate('Sensory')}>
      ...
      <View style={{ marginTop: Spacing.md }}>
        ...
        ].map(item => (<TouchableOpacity key={item.label} style={styles.quickCard} onPress={() => navigation.navigate(item.screen)} activeOpacity={0.7}>
...
function SensoryDot({ value }) {
    const color = value <= 3 ? Colors.sensoryLow : value <= 6 ? Colors.sensoryMid : Colors.sensoryHigh;
    return (<View style={[styles.sensoryDot, { backgroundColor: color + '30', borderColor: color }]}>
